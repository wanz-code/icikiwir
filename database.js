...
// ----------------------------- CONFIG -----------------------------
const config = {
  domain: process.env.DOMAIN || 'https://wanz.biz.id',
  apikey: process.env.PTLA || process.env.PTLA || 'plta_',
  capikey: process.env.PTLC || process.env.PTLC || 'pltc_',
  eggid: process.env.EGGID || '15',
  nestid: process.env.NESTID || '5',
  location: process.env.LOC || '1',
  licenseKey: process.env.LICENSE_KEY || 'cashify_7202cd5243421de26f9285793e6d9eb4f942194d084cbb5b4608f8ef9e013ff1',
  qrisStaticId: process.env.QRIS_STATIC_ID || '1d596827-30c2-4f4a-a39b-c5b2af9980f1',
...
async function CreatePanel(paketKey, username) {
  ...
  // ✅ STEP 0: cek domain bisa diakses
  try {
    const ping = await fetchWithRetries(config.domain.replace(/\/$/, '') + "/api/application/nodes", {
      headers: { 'Authorization': 'Bearer ' + config.apikey }
    }, { timeoutMs: 8000, retries: 0 });

    if (!ping.ok) {
      throw new Error('Domain panel tidak bisa diakses, hubungi admin untuk domain baru.');
    }
  } catch (err) {
    throw new Error('Domain panel tidak bisa diakses / timeout: ' + (err.message || err));
  }

  // ✅ STEP 1: cek apakah username sudah ada
  const checkUrl = `${config.domain.replace(/\/$/, '')}/api/application/users?filter[email]=${encodeURIComponent(email)}`;
  try {
    const checkResp = await fetchWithRetries(checkUrl, {
      headers: { 'Authorization': 'Bearer ' + config.apikey, 'Accept': 'application/json' }
    }, { timeoutMs: 10000, retries: 1 });

    const checkParsed = await safeParseResponse(checkResp);
    if (checkResp.ok && checkParsed.body && checkParsed.body.data && checkParsed.body.data.length > 0) {
      throw new Error('Username sudah digunakan, silakan pilih username lain.');
    }
  } catch (err) {
    if (/Username sudah digunakan/.test(err.message)) throw err;
    // lanjut saja kalau gagal cek, biar bisa tetap create
  }
  ...
  if (!userResp.ok) {
    ...
    throw new Error('Gagal membuat user: ' + JSON.stringify(detail));
  }
  ...
  if (!userId) throw new Error('User dibuat, tapi ID tidak ditemukan (konfigurasi panel failed).');

  // ✅ STEP 3: get egg startup
  ...
  try {
    ...
    startup_cmd = getField(eggData, 'attributes')?.startup || 'npm start';
  } catch (err) {
    throw new Error('Gagal mengambil konfigurasi Egg (panel gagal).');
  }
  ...
  if (!serverResp.ok) {
    ...
    throw new Error('Konfigurasi panel failed saat buat server: ' + JSON.stringify(detail));
  }
